...
def log2_graph(x):
    ...
    #변경점(텐서플로우 버전 업그레이드로 인한 변경 tf.log -> tf.math.log)
...
class PyramidROIAlign(KL.Layer):
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            #변경점 tensorflow2.x에서 tensorflow1.x 버전 함수 사용을 위해
            #자동으로 tensorflow1.x 버전의 코드를 2버전에서도 쓸수있게됨.
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    #변경점 tensorflow(2.x) 버전에서 1.x버전의 메소드 사용을 위해
    #compat.v1 추가
    #이후에는 2.x버전에서 1.x버전을 위해 메소드 사용이라고 서술
    ...
    #변경점 2.x버전에서 1.x버전을 사용 위해 compat.v1 메소드 사용
    ...
    #변경점 2.x버전에서 1.x버전을 위해 compat.v1 메소드 사용
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        ...
        #변경점 index 유효성 검사에서 계속 에러가 터져 아예 검사하지 않는 메소드 사용
        #tf.sparse_tensor_to_dense()에서 tf.sparse.to_dense로 변경
    ...
    #변경점 2.x버전에서 1.x버전을 사용하기 위해 compat.v1 메소드 사용
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    s = K.int_shape(x)
    #변경점 자꾸 s[1]가 none으로 처리가 되어 해당 s[1]이 none 인경우 고정 값을 넣도록 수정
    if s[1] is None:
...
#  Data Generator
############################################################
#변경점 이후에 use_mini_mask=False을 이후에 넣었을때 오류가 나 제거 
